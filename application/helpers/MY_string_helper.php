...
if ( ! function_exists('array2option2'))
{
	function array2option2($arraydata, $selectid = '')
	{
		if (!is_array($arraydata)) return $arraydata;
		$retstr1 = '';
		$retstr = array();
		...
		foreach ($retstr[$selectid] as $k => $v)
		{   if($v['level'] != 0)
			$retstr1 .= "<option value='" . $v['id'] . "'" . '' . ">" . $v['name'] . "</option>";
		}
		return $retstr1;
	}
}
 ...
 * 创建一个文档目录
 */
if ( ! function_exists('cate2child'))
{
	function cate2child($arr,$pid = 0,$level = 0)
	{
		static $cat_child = array();
		foreach ($arr as $k => $v) {
			//说明找到了以$pid为父节点的子节点,将其保存
			if ($v['parentid'] == $pid && $v['parentid'] == 0){
				$v['level'] = $level;
				$v['child'] = _tree($arr,$v['cateid'],$level + 1);	
				$cat_child[] = $v;
...
{
	function _tree($arr,$pis,$level = 1)
	{
		$cat_tree = array();
			//查找父节点不为顶级节点的字节点，找到父节点为pic的子节点。并返回
			foreach ($arr as $k => $v) {
