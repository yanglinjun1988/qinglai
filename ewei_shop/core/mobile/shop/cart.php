...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $data          = pdo_fetch("select {$datafields} from " . tablename('ewei_shop_member_cart') . ' where openid=:openid and goodsid=:id and  optionid=:optionid and deleted=0 and  uniacid=:uniacid   limit 1', array(
            ':uniacid' => $uniacid,
            ':openid' => $openid,
            ':optionid' => $optionid,
            ':id' => $id
        ...
        if (empty($data)) {
            ...
            pdo_update('ewei_shop_member_cart', $data, array(
                'id' => $data['id']
            ));
        }
        show_json(1, array(
