...
if ($_W["isajax"]) {
    ...
    $counts = array(
        "cartcount" => pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("ewei_shop_member_cart") . " where uniacid=:uniacid and openid=:openid and deleted=0 ", array(
            ":uniacid" => $uniacid,
            ":openid" => $openid
        )),
        "favcount" => pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_member_favorite") . " where uniacid=:uniacid and openid=:openid and deleted=0 ", array(
            ":uniacid" => $uniacid,
            ":openid" => $openid
        ))
    );
    if ($plugin_coupon) {
        ...
        $counts["couponcount"] = pdo_fetchcolumn($sql, array(
            ":openid" => $openid,
            ":uniacid" => $_W["uniacid"]
        ));
    }
