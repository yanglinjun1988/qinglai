...
function zymfunc_3($zym_var_32) {
    ...
    if (!empty($zym_var_32["refundid"])) {
        $zym_var_35 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id limit 1", array(
...
function zymfunc_1($zym_var_32) {
    ...
    pdo_update("ewei_shop_order", $zym_var_36, array(
        "id" => $zym_var_32["id"],
        "uniacid" => $_W["uniacid"]
    ));
    if (!empty($zym_var_32["refundid"])) {
...
function order_list_refund($zym_var_32) {
    ...
    $zym_var_35 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id and status=0 limit 1", array(
        ":id" => $zym_var_32["refundid"]
    ));
    if (empty($zym_var_35)) {
        pdo_update("ewei_shop_order", array(
    ...
    if ($zym_var_27 == 0) {
        ...
        $zym_var_18 = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename("ewei_shop_order_goods") . " o left join " . tablename("ewei_shop_goods") . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(
        ...
        foreach ($zym_var_18 as $zym_var_23) {
            $zym_var_24 = pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid " . " where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1", array(
