...
if ($operation == 'display') 
{
	...
	$sql = 'SELECT * FROM ' . tablename('ewei_shop_coupon') . ' ' . " where  1 and {$condition}
ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
...
unset($row);
$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_coupon') . " where 1 and {$condition}
", $params);
...
if (!empty($id)) 
{
	...
	plog('coupon.coupon.edit', "编辑优惠券 ID: {$id}
<br/>优惠券名称: {$data['couponname']}
");
}
else 
{
...
$id = pdo_insertid();
plog('coupon.coupon.add', "添加优惠券 ID: {$id}
 <br/>优惠券名称: {$data['couponname']}
");
}
...
plog('coupon.coupon.delete', "删除优惠券 ID: {$id}
 <br/>优惠券名称: {$item['couponname']}
");
...
$time = time();
$ds = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_coupon') . "  WHERE 1 {$condition}
