...
if ($operation == 'display') {
    ...
    $id    = intval($_GPC['id']);
    $apply = pdo_fetch('select * from ' . tablename('ewei_shop_commission_apply') . ' where uniacid=:uniacid and id=:id limit 1', array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if (empty($apply)) {
    ...
    if (checksubmit('submit_check') && $apply['status'] == 1) {
        ...
        $ogids         = array();
        foreach ($list as $row) {
            $goods = pdo_fetchall("SELECT id from " . tablename('ewei_shop_order_goods') . " where uniacid = :uniacid and orderid=:orderid and nocommission=0", array(
                ':uniacid' => $_W['uniacid'],
                ':orderid' => $row['id']
            ));
            foreach ($goods as $g) {
        ...
        foreach ($ogids as $ogid) {
            $g = pdo_fetch("SELECT total, commission1,commission2,commission3,commissions from " . tablename('ewei_shop_order_goods') . "  " . "where id=:id and uniacid = :uniacid limit 1", array(
                ':uniacid' => $_W['uniacid'],
                ':id' => $ogid
            ));
            if (empty($g)) {
            ...
            if (!empty($update)) {
                pdo_update('ewei_shop_order_goods', $update, array(
                    'id' => $ogid
                ));
            }
        }
